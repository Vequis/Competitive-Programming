...
int subsetSum(int a[], int n, int sum)
{
    if (sum == 0)
        return 1;
    //Se chegou na soma alvo, retorna true;

    if (n <= 0)
        return 0;
    //Se já rodou em todos os números, retorna false;
    ...
    if (a[n - 1] > sum)
        //Se o elemento que estamos analisando for maior que a soma, não poderemos utilizá-lo;
        return tab[n - 1][sum] = subsetSum(a, n - 1, sum);
    else
    {
        //Se não, o elemnto pode ser considerado na soma, ou não, e portanto devemos analisar os dois casos.
